...
router.put('/me', authMiddleware, upload.single('avatar'), async (req, res) => {
    ...
    try {
        ...
        if (file) {
            // Salva apenas o caminho relativo (/uploads/nome-do-arquivo)
            // O frontend vai concatenar com API_URL conforme necessário
            dataToUpdate.avatarUrl = `/uploads/${file.filename}`;
        }
